...
class GPT(nn.Module):
    ...
    def forward(self, context: torch.Tensor) -> torch.Tensor:
        ...
        embeddings = self.token_embeddings(context) + self.positional_embeddings(
            positions
        )  # [batch x context_length x model_dim]

        transformers_out = self.transformers(
            embeddings
        )  # all transformer blocks executed sequentially
